...
| authors | string(255)[] | NO | | 著者名の配列（Google Books API のレスポンスをそのまま保存）|
...
- `authors` は PostgreSQL の配列型（`varchar(255)[]`）で保存します。Google Books API のレスポンスが配列形式のためそのまま保存でき、Rails・フロントエンドともに配列として扱えます。Rails migration では `array: true` を指定し、要素の型は `varchar(255)` とします（著者名1件あたり最大255文字）
